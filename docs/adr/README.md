...
| [0037](0037-reference-data-complexity-tiers-and-layering.md) | Reference-data complexity tiers — simple lookups stay single-step/flat; complex (hierarchy / multi-grain) reference data uses raw→processed layering (raw + intermediate in the source catalog, canonical promoted to the model catalog), adding per-level tables without dropping the flat consumer table. Amends 0014/0030; extends 0036 (multi-stage builder). Proving grounds: geography BG+block (multi-grain) + ICD-10-CM relayered (hierarchy) | Proposed |
