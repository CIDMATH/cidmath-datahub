...
| [0040](0040-orchestration-granularity.md) | Pipeline orchestration granularity — orchestration lives in the job DAG (`depends_on`), computation in the builder; **unit of orchestration = the build (per level), parents-first**. The **capture (source→raw) vs derive (raw→canonical) seam** becomes a *task* boundary only when the two have **different triggers**: cadence (revise-in-place capture vs derive), cross-subject dependency on derive (augmenting inputs, fact→dimension conformance), propagation/re-derive on upstream change (re-capturing would mint spurious snapshots — a correctness issue), or compute/governance. All-shared (stable source, uniform compute, no gate) → combined. **Geography (root dimension, immutable) stays combined per-level**; RUCA/SVI/facts are the consumer-side splits. Builds on 0036/0027/0037/0039/0032 | Proposed |
| [0041](0041-cross-subject-lineage-and-source-revisions.md) | Cross-subject lineage, propagation & source revisions — closes the gaps 0040 left implicit (it assumed an intra-job DAG + immutable vintages). **D1** declare/order/assert cross-job edges (manifest + `Run Job` hard edges + a parent-exists-at-expected-`(vintage,revision)` precondition); **D2** record build lineage (`table_version`+`content_fingerprint` per table, `conformance_lineage` per consumer) so propagation can actually fire + staleness is detectable not silent; **D3** source revisions first-class (Volume fingerprint drives skip-if-complete; re-release bumps `vintage_revision`; immutability per `(vintage,revision)`); **D4** gate atomic-across-vintages + downstream blocks on freshness vs reads stale; **D5** Volume class (re-creatable vs history-of-record) governs `DROP`/retention; **D6** `max_concurrent_runs:1` for revise-in-place capture; **D7** skip provably-unchanged rebuilds (skip derive iff build-signature {source fingerprint + derive code/version + parent versions} unchanged — NOT "rows exist", which would freeze the build against code/upstream changes). Inert until RUCA (first cross-job consumer) + first fact. Extends 0040/0039/0034/0032/0037/0035 | Proposed |
