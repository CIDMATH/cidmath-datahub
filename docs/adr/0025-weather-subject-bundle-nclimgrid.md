...
**Implementation note (2026-06-30, Volume landing retrofit — ADR 0039).** The raw slice originally downloaded each monthly CSV and parsed it directly (no verbatim copy). It now **lands each payload verbatim in a `weather_raw._landing` UC Volume before parsing** (ADR 0039), and builds the raw rows from the landed file — so the raw table is reproducible from stored payloads without re-fetching NOAA, which revises `prelim→scaled` and rewrites files (a source that mutates is exactly the case ADR 0039 wants captured). Retention is **SNAPSHOT_PER_RUN** (`snapshot_date=<run>/`): each run captures a fresh verbatim snapshot, mirroring the `merge_upsert` table semantics. The Volume is engineer-only; the build grants READ/WRITE VOLUME to the running principal + engineer group (ownership does not confer FUSE file access), same as the shared reference builder's Phase 0. Weather uses `run_build` (not the `build_reference` builder), so the landing is hand-rolled rather than a `RawLanding`, but the principle is identical.
