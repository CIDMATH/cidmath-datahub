...
### 10. Bundles that share a `bundle.name` collide on one workspace root

**What we hit.** A `_reference` deploy's `build_time` job failed mid-run with `Cannot read the python file ... build_time.py`. The file had been there moments before. Root cause: `databricks-common.yml` set a single `bundle.name: cidmath-datahub`, and because every bundle includes that shared file, its value overrode each bundle's own name. So `_platform` and `_reference` both deployed to the *same* workspace root, `/Workspace/Users/<sp>/.bundle/cidmath-datahub/dev/`. A push that changes shared code (`src/cidmath_datahub/**`, `databricks-common.yml`) triggers *both* deploy workflows, which run concurrently (their concurrency groups are keyed per-workflow, so — unlike lesson #9 — they don't cancel each other). When `_platform`'s `bundle deploy` synced its file set into the shared `files/`, it pruned files not in its set (`bundles/_reference/src/build_time.py`) out from under the running `_reference` job.

**What we did.** Removed `bundle.name` from `databricks-common.yml` and let each bundle's own unique name take effect (`cidmath-datahub-platform`, `cidmath-datahub-reference`). Each now deploys to its own root (`.bundle/cidmath-datahub-platform/dev`, `.bundle/cidmath-datahub-reference/dev`), so their syncs can't touch each other. This also fixed the same latent collision in prod, where `root_path` is built from `${bundle.name}`. Renaming orphaned the old shared-name deployment (jobs + workspace folder under `.bundle/cidmath-datahub/`), which needed a one-time manual cleanup.

**Lesson.** Never set `bundle.name` in a shared/included config used by more than one bundle — each bundle needs a unique name, which is what gives it an isolated workspace root. The per-workflow concurrency groups from lesson #9 do *not* protect against this, because the racing deploys belong to different workflows. When multiple bundles can deploy off one push, confirm each lands at a distinct `.bundle/<name>/<target>` path.
