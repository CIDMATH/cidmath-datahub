...
- `geography.state` — PK `(geoid, vintage)`; `geoid` (2-digit), `gisjoin`, `name`, `stusps` (e.g., GA), `hhs_region`, `centroid_geo_lon/lat` + `centroid_pop_lon/lat`, land/water area.
- `geography.county` — PK `(geoid, vintage)`; `geoid` (5-digit), `state_geoid` (FK), `gisjoin`, `name`, `centroid_geo_lon/lat` + `centroid_pop_lon/lat`, land/water area.
- `geography.zcta` — PK `(geoid, vintage)`; `geoid` (5-digit), `gisjoin`, `centroid_geo_lon/lat` (geographic only — no Centers of Population for ZCTAs), area.
- `geography.tract` — PK `(geoid, vintage)`; `geoid` (11-digit), `county_geoid` + `state_geoid` (FK), `gisjoin`, `centroid_geo_lon/lat` + `centroid_pop_lon/lat`, area.
- `geography.cbsa` — PK `(geoid, vintage)`; `geoid` (5-digit CBSA code), `gisjoin`, `name`, type (metro/micro), `centroid_geo_lon/lat` (geographic only).
- `geography.hhs_region` — PK `hhs_region` (1-10), `name` (static lookup; `geography.state` carries the `hhs_region` membership).

**Storage guardrails:** `geoid` and `gisjoin` are stored as `STRING`, never integer — leading zeros are significant (Alabama is `01`, not `1`). `cbsa_geoid` on `geography.county` is nullable (rural counties belong to no CBSA). Each entity stores two centroid pairs: `centroid_geo_lon`/`centroid_geo_lat` (geographic interior point, always set) and `centroid_pop_lon`/`centroid_pop_lat` (population-weighted, set only where NHGIS provides Centers of Population — state/county/tract — else null).
...
**Implementation refinement (2026-05-21).** During slice 1 wiring two scope adjustments were made. First, the **county crosswalk is deferred to slice 2**: NHGIS publishes no direct county→county crosswalk (its 2010↔2020 crosswalks are sourced *from block groups*, and deriving a true county→county weight set requires population denominators that live in the `population` schema), and counties are near-stable across the decade, so the crosswalk pattern is better introduced at the redrawn tract/block-group/ZCTA levels. Second, the entity tables carry **two centroid pairs**: a geographic interior point (`centroid_geo_lon`/`centroid_geo_lat`, always populated, via shapely `representative_point`) and a population-weighted center (`centroid_pop_lon`/`centroid_pop_lat`, populated where the Census Centers of Population cover the unit — state/county/tract — and null otherwise, e.g. ZCTA). The `cenpop` point shapefiles are pulled in the same NHGIS extract and joined by GISJOIN. (This supersedes the earlier single-centroid-plus-flag sketch above: both centroids are stored side by side rather than one column plus a `centroid_is_pop_weighted` flag.) State identity (USPS, name, HHS region) is derived in code from a small canonical FIPS table rather than from shapefile attributes, so the build is robust to NHGIS attribute-naming variation.
