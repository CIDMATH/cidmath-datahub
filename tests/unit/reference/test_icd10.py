"""Unit tests for `cidmath_datahub.reference.icd10`.
...
class TestNormalizeCode:
    ...
    def test_normalization(self, raw, expected):
        assert icd10.normalize_code(raw) == expected
...
class TestValidateCode:
    ...
    def test_invalid(self, code):
        assert icd10.validate_code(code) is False


@pytest.mark.unit
class TestParseOrderLine:
    def test_billable_leaf(self):
        rec = icd10.parse_order_line(SAMPLE_LINES[3], EDITION)  # U07.1
        assert rec is not None
        assert rec.icd10_code == "U07.1"
    ...
    def test_header_not_billable(self):
        rec = icd10.parse_order_line(SAMPLE_LINES[0], EDITION)  # A00 header
        assert rec is not None
        assert rec.icd10_code == "A00"
        assert rec.is_billable is False

    def test_blank_and_short_lines_skipped(self):
        assert icd10.parse_order_line("", EDITION) is None
        assert icd10.parse_order_line("   ", EDITION) is None
        assert icd10.parse_order_line("00001 A00", EDITION) is None  # too short for a flag
    ...
    def test_bad_flag_raises(self):
        ...
        with pytest.raises(ValueError, match="Unexpected billable flag"):
            icd10.parse_order_line(bad, EDITION)


@pytest.mark.unit
class TestParseOrderFile:
    def test_parses_all_content_lines(self):
        recs = icd10.parse_order_file(SAMPLE_FILE, EDITION)
        assert len(recs) == 4

    def test_known_codes_present_and_dotted(self):
        recs = icd10.parse_order_file(SAMPLE_FILE, EDITION)
        by_code = {r.icd10_code: r for r in recs}
...
class TestDQHelpers:
    def test_find_format_violations(self):
        recs = icd10.parse_order_file(SAMPLE_FILE, EDITION)
        assert icd10.find_format_violations(recs) == []  # clean sample
        bad = recs + [icd10.Icd10Record("bogus!", EDITION, "x", True, "x")]
        assert icd10.find_format_violations(bad) == ["bogus!"]

    def test_find_missing_descriptions(self):
        recs = icd10.parse_order_file(SAMPLE_FILE, EDITION)
        assert icd10.find_missing_descriptions(recs) == []  # all populated
        blank = recs + [icd10.Icd10Record("Z99.9", EDITION, "  ", True, "")]
        assert icd10.find_missing_descriptions(blank) == [("Z99.9", EDITION)]

    def test_find_duplicate_keys(self):
        recs = icd10.parse_order_file(SAMPLE_FILE, EDITION)
        assert icd10.find_duplicate_keys(recs) == []  # unique per edition
        dup = recs + [recs[0]]  # repeat A00 for the same edition
        assert icd10.find_duplicate_keys(dup) == [("A00", EDITION)]

    def test_same_code_different_editions_not_duplicate(self):
        a = icd10.Icd10Record("U07.1", 2021, "COVID-19", True, "COVID-19")
        b = icd10.Icd10Record("U07.1", 2025, "COVID-19", True, "COVID-19")
        assert icd10.find_duplicate_keys([a, b]) == []


@pytest.mark.unit
class TestSourceLocators:
    def test_order_file_zip_url_default_template(self):
        url = icd10.order_file_zip_url(2026)
    ...
    def test_order_file_zip_url_custom_template(self):
        url = icd10.order_file_zip_url(2021, template="file:///data/icd10/{year}/order-{year}.zip")
        assert url == "file:///data/icd10/2021/order-2021.zip"

    def test_update_file_zip_url_default_template(self):
        url = icd10.update_file_zip_url(2026)
    ...
    def test_select_order_file_member_picks_order_file(self, name):
        ...
        assert icd10.select_order_file_member(members) == name
    ...
    def test_select_order_file_member_rejects_codes_only(self):
        ...
        with pytest.raises(ValueError, match="order file"):
            icd10.select_order_file_member(["icd10cm-codes-2026.txt", "readme.txt"])
...
class TestOverlayRecords:
    ...
    def _base(self):
        return [
            icd10.Icd10Record("A00", 2026, "Cholera", False, "Cholera"),
            icd10.Icd10Record("J18.9", 2026, "Pneumonia, unspecified organism", True, "Pneumonia"),
            icd10.Icd10Record("U07.1", 2026, "COVID-19", True, "COVID-19"),
        ]

    def test_empty_update_returns_base(self):
        base = self._base()
        assert icd10.overlay_records(base, []) == base

    def test_update_revises_existing_code(self):
        base = self._base()
        update = [icd10.Icd10Record("J18.9", 2026, "Pneumonia, unspecified (revised)", True, "Pn")]
        merged = {r.icd10_code: r for r in icd10.overlay_records(base, update)}
    ...
    def test_update_adds_new_code(self):
        base = self._base()
        update = [icd10.Icd10Record("U07.2", 2026, "Vaping-related disorder", True, "Vaping")]
        merged = {r.icd10_code: r for r in icd10.overlay_records(base, update)}
        assert len(merged) == 4
    ...
    def test_base_only_codes_retained_and_keys_unique(self):
        base = self._base()
        update = [
            icd10.Icd10Record("A00", 2026, "Cholera (rev)", False, "Cholera"),
            icd10.Icd10Record("B99.9", 2026, "Unspecified infectious disease", True, "Inf"),
        ]
        merged = icd10.overlay_records(base, update)
        codes = [r.icd10_code for r in merged]
...
class TestTabularSourceLocators:
    ...
    def test_select_tabular_xml_member(self, name):
        ...
        assert icd10.select_tabular_xml_member(members) == name

    def test_select_tabular_xml_member_rejects_index_only(self):
        with pytest.raises(ValueError, match="tabular"):
            icd10.select_tabular_xml_member(["icd10cm-index-2026.xml", "readme.txt"])
...
class TestPrefixesAndAncestors:
    ...
    def test_code_prefixes(self, code, expected):
        assert icd10.code_prefixes(code) == expected

    def test_category_of(self):
        assert icd10.category_of("S72.001A") == "S72"
        assert icd10.category_of("A00") == "A00"

    def test_ancestors_only_existing_prefixes(self):
        code_set = {"S72", "S72.0", "S72.00", "S72.001", "S72.001A"}
        assert icd10.ancestors_for("S72.001A", code_set) == ["S72", "S72.0", "S72.00", "S72.001"]
    ...
    def test_ancestors_skip_x_placeholder_stems(self):
        ...
        assert icd10.ancestors_for("S02.0XXA", code_set) == ["S02", "S02.0"]


@pytest.mark.unit
class TestTabularCategoryMap:
    def test_maps_categories_with_clean_labels(self):
        m = icd10.parse_tabular_category_map(TABULAR_XML)
...
class TestBuildHierarchy:
    ...
    def test_deep_leaf_full_chain(self):
        cm = icd10.parse_tabular_category_map(TABULAR_XML)
        nodes = {n.icd10_code: n for n in icd10.build_hierarchy(self._records(), cm)}
        n = nodes["S72.001A"]
        assert n.parent_icd10_code == "S72.001"
    ...
    def test_category_root_has_null_parent(self):
        cm = icd10.parse_tabular_category_map(TABULAR_XML)
        nodes = {n.icd10_code: n for n in icd10.build_hierarchy(self._records(), cm)}
        assert nodes["U07"].parent_icd10_code is None
        assert nodes["U07"].node_level == 0
        assert nodes["U07.1"].parent_icd10_code == "U07"
    ...
    def test_empty_category_map_leaves_chapter_block_null(self):
        nodes = icd10.build_hierarchy(self._records(), {})
        # adjacency still computed from the code set even with no XML
        by = {n.icd10_code: n for n in nodes}
        assert by["S72.001A"].parent_icd10_code == "S72.001"
    ...
    def test_unmapped_and_orphan_dq_helpers(self):
        ...
        recs = [icd10.Icd10Record("B99.9", 2026, "Unspecified infectious disease", True, "")]
        nodes = icd10.build_hierarchy(recs, cm)
        assert icd10.find_unmapped_categories(nodes) == ["B99"]
        assert icd10.find_orphan_codes(nodes) == ["B99.9"]
        # the clean sample has neither
        clean = icd10.build_hierarchy(self._records(), cm)
        assert icd10.find_unmapped_categories(clean) == []
        assert icd10.find_orphan_codes(clean) == []
...
class TestTabularTree:
    ...
    def test_parent_of_from_nesting(self):
        tree = icd10.parse_tabular_tree(TABULAR_XML)
...
class TestXmlSourcedAdjacency:
    ...
    def test_listed_node_parent_from_xml(self):
        ...
        nodes = {
            n.icd10_code: n
            for n in icd10.build_hierarchy(self._records(), tree.category_map, tree.parent_of)
        }
        # S72.001 is an XML node: parent/ancestors read straight from the nesting
        assert nodes["S72.001"].parent_icd10_code == "S72.00"
    ...
    def test_resolve_ancestors_direct(self):
        tree = icd10.parse_tabular_tree(TABULAR_XML)
        code_set = {r.icd10_code for r in self._records()}
        assert icd10.resolve_ancestors("S72.001", tree.parent_of, code_set) == [
            "S72",
            "S72.0",
            "S72.00",
        ]
        # empty parent_of (XML skipped) -> prefix rule over the code set
        assert icd10.resolve_ancestors("S72.001A", {}, code_set) == [
            "S72",
            "S72.0",
            "S72.00",
            "S72.001",
    ...
    def test_adjacency_cross_check(self):
        ...
        assert icd10.find_adjacency_mismatches(recs, tree.parent_of) == []
        # corrupt one XML parent -> the cross-check flags exactly that code
        bad = dict(tree.parent_of)
        bad["S72.0"] = "S99"
        assert icd10.find_adjacency_mismatches(recs, bad) == ["S72.0"]
