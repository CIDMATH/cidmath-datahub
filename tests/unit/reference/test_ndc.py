...
class TestDedupe:
    ...
    def test_dedupe_packages_keeps_latest_start_marketing_date(self):
        # Same product_id + package code re-listed with a newer STARTMARKETINGDATE
        # (the real FDA case): keep the most recent listing, not the file's first row.
        rows = (
            "PRODUCTID\tPRODUCTNDC\tNDCPACKAGECODE\tPACKAGEDESCRIPTION\t"
            "STARTMARKETINGDATE\tENDMARKETINGDATE\tNDC_EXCLUDE_FLAG\tSAMPLE_PACKAGE\n"
            "72043-2500_x\t72043-2500\t72043-2500-1\t48 g in 1 BOTTLE\t20180110\t\t\tN\n"
            "72043-2500_x\t72043-2500\t72043-2500-1\t48 g in 1 BOTTLE\t20250501\t\t\tN\n"
        )
        deduped, dropped = ndc.dedupe_packages(ndc.parse_package_file(rows))
        assert len(deduped) == 1 and dropped == 1
        assert deduped[0].start_marketing_date == date(2025, 5, 1)
