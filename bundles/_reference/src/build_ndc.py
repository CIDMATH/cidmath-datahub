...
_KNOWN_LIMITATIONS = (
    ...
    "dates carry real-world validity. The FDA package file occasionally re-lists the same "
    "package code under one SPL with a newer start marketing date; such duplicates are "
    "collapsed to the most recent listing per (product_id, package code) (earlier listings "
    "remain in the raw Volume snapshot). Some packages reference a product absent from "
    "product.txt -- the package->product FK is informational (ADR 0014), so such rows are "
    "kept. PHARM_CLASSES / SUBSTANCENAME are "
